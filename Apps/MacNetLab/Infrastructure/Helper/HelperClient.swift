...
actor HelperClient {
    ...
    private let logger = Logger(subsystem: "com.bee.macnetlab", category: "helper-client")
    ...
    func install() throws(ServiceFailure) -> HelperInstallationState {
        do {
            ...
            throw ServiceFailure(
                ...
                recoverySuggestion:
                    "Make sure MacNetLab is in your Applications folder, then try again.",
    ...
    private func withProxy(
        ...
        do {
            return try await withCheckedThrowingContinuation { continuation in
                ...
                guard let helper = proxy as? any MacNetLabHelperProtocol else {
                    box.resume(with: .failure(ServiceFailure.internalError(
                        "helper proxy does not conform to MacNetLabHelperProtocol"
                    )))
                    return
                }
    ...
    private static func transportFailure(from error: NSError) -> ServiceFailure {
        ServiceFailure(
            ...
            message: "MacNetLab could not communicate with its privileged helper.",
            recoverySuggestion:
...
private final class HelperEventReceiver: NSObject, MacNetLabHelperClientProtocol {
    private let logger = Logger(subsystem: "com.bee.macnetlab", category: "helper-events")
