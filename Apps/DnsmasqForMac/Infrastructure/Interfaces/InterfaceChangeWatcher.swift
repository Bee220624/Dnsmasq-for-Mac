...
final class InterfaceChangeWatcher: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.bee.dnsmasqformac", category: "interface-watcher")
    private let queue = DispatchQueue(label: "com.bee.dnsmasqformac.interface-watcher")
    ...
    private func startNotifications() -> Bool {
        ...
        guard let store = SCDynamicStoreCreate(
            nil,
            "com.bee.dnsmasqformac.interface-watcher" as CFString,
            callback,
            &context
        ) else { return false }
