    ...
    var body: some View {
        Section("dnsmasq Engine") {
            if let engine = connectedEngine {
                ...
                Text("Reinstall Dnsmasq for Mac from your original download.")
                    .font(.callout)
...
struct LicensesSection: View {
    ...
    var body: some View {
        Section("Licenses") {
            ...
            Text("Dnsmasq for Mac bundles dnsmasq by Simon Kelley as a separate, unmodified program. No dnsmasq code is compiled or linked into Dnsmasq for Mac itself.")
                .font(.callout)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
