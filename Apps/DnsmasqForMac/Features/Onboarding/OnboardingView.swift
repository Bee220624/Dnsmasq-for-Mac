...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("Dnsmasq for Mac needs a privileged helper")
                .font(.title2.weight(.semibold))

            Text("Serving DHCP and DNS means adding a temporary address to a network interface and listening on ports 53 and 67. Only a privileged helper can do that. Dnsmasq for Mac itself never runs with administrator rights.")
                .font(.callout)
            ...
            VStack(alignment: .leading, spacing: 6) {
                Label("The helper starts only when Dnsmasq for Mac asks it to.", systemImage: "bolt.slash")
    ...
    private var stateSpecificContent: some View {
        ...
        case .notInstalled(.requiresApproval):
            ...
            VStack(spacing: 10) {
                ...
                Text("Open Login Items & Extensions, then enable Dnsmasq for Mac. This window updates on its own once you do.")
                    .font(.callout)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
                    .fixedSize(horizontal: false, vertical: true)
