...
macOS で dnsmasq を使いたくて、GUI のあるものを探しました。見つかったのは 1 つだけ、しかも有料でした。

要するに dnsmasq にウィンドウを被せるだけのものです。お金を取るような話ではないだろうと思い、自分で
作ってオープンソースにしました。
...
- 設定はプロファイルとして保存・切り替え
- システム設定を一切変更しない——`/etc/hosts` にも `/etc/dnsmasq.conf` にも触れず、恒久的な変更を
  残しません。テレメトリなし、実行時のネットワーク通信なし、自動起動なし
- Wi-Fi と現在インターネット接続を担っているインターフェースでは DHCP を禁止。UI 上のグレーアウト
  だけでなく特権ヘルパー側で強制されます
